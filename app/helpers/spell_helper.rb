module SpellHelper
  def edit_confirm_div
    content_tag(:span, class: 'edit-confirm') do
      bs_glyph('check-circle', class: %w(btn btn-xs btn-default edit-okay)) <<
        bs_glyph('minus-circle', class: %w(btn btn-xs btn-default edit-nope))
    end
  end
  ...
  def edit_basic_header(object, field_name)
    content_tag(:div, class: 'form-control-static') do
      content_tag(:span, object.send(field_name), 'data-static-attribute' => field_name) <<
        bs_glyph('edit', id: 'spell_name_edit_icon', 'aria-hidden' => true, class: 'edit-icon')
    end
  end
  ...
  def edit_redirect_header(object, field_name, method)
    content_tag(:div, class: %w(form-control-static)) do
      tags = []
      Array.wrap(field_name).each do |fn|
      ...
      tags << bs_glyph('edit', id: 'spell_name_edit_icon', 'aria-hidden' => true, class: 'edit-icon')
      safe_join(tags)
    end
  end

  def edit_field_row(widths, &block)
