function characters_open_edit_form(ev) {
  let $modal = $( '#editchar-modal' ).modal( 'show' );
  let $form = $( 'form', $modal );
  let $me = $( ev.currentTarget );
  let $this_row = $me.parents( 'tr' );
  let char_id = $this_row.attr( 'data-char-id' );
  let $these_data = $this_row.children( 'td' );
  $form.attr( 'action', '/characters/' + char_id );
  let fields = [ "name", "str", "dex", "con", "int", "wis", "chr",
      "perception", "initiative", "speed", "ac" ];
  for (let i = 0; i < fields.length; i++) {
    objname = 'input[name="character[' + fields[i] + ']"]';
    $( objname, $form ).val( $these_data.eq(i+1).text() );
  }
  let $notes_row = $this_row.next();
  if ( $notes_row.attr( 'data-notes-id' ) == char_id ) {
    $( 'input[name="character[notes]"]', $form ).val( $notes_row.find( 'td' ).eq(0).text() );
  }
  $( 'input[name="character[highlight]"]', $form )
    .prop( 'checked', ( $this_row.attr( 'data-char-highlight' ) == "true" ) );
}
...
function characters_fetch() {
  $.ajax({
    ...
    success: function(data) {
      ...
      $( '.fa-edit', '#character-table' ).on( 'click', characters_open_edit_form );
    }
  })
}
...
$( function () {
  ...
  let $editchar_form = $( '#editchar-form' );
  let $editchar_inputs = $( 'input.form-control', $editchar_form );
